...
public class StandardCheckoutProcessor extends CheckoutTemplate {
    ...
    private static final BigDecimal DEFAULT_SHIPPING_FEE = BigDecimal.valueOf(20000);
    ...
    protected void prepareAddress(CheckoutContext context) {
        ...
        if (chosenAddress.getUser() == null
                || !chosenAddress.getUser().getId().equals(context.getUser().getId())) {
            throw new IllegalArgumentException("Dia chi giao hang khong thuoc nguoi dung hien tai.");
        }

        Address orderAddress = createOrderAddressSnapshot(chosenAddress);
        context.setOrderAddress(addressRepository.save(orderAddress));
    }
    ...
    private Address createOrderAddressSnapshot(Address source) {
        ...
        snapshot.setUser(source.getUser());
        return snapshot;
    }
}
