...
import com.alotra.repository.user.CustomerRepository;
...
public class AuthController {
	...
	public ResponseEntity<?> registerUser(@Valid @RequestBody SignUpDto signUpDto, BindingResult result) {
		...
		try {
			...
			user.setStatus((byte) 0); // 0 = chưa xác thực
	...
	public ResponseEntity<?> verifyOtp(@Valid @RequestBody OtpVerificationDto dto, BindingResult result) {
		...
		user.setStatus((byte) 1); // Active = đã xác thực
	...
	public ResponseEntity<?> authenticateUser(@Valid @RequestBody LoginDto loginDto, BindingResult result,
		...
		try {
			...
			if (user.getStatus() == 0) {
				return ResponseEntity.badRequest()
						.body(Map.of("errorCode", "USER_NOT_VERIFIED", "message", "Tài khoản chưa được xác thực!"));
			}

			if (user.getStatus() == 2) {
				return ResponseEntity.badRequest()
						.body(Map.of("errorCode", "USER_INACTIVE", "message", "Tài khoản đã bị vô hiệu hóa!"));
			}
			...
			// Tạo security context và session
			SecurityContextHolder.getContext().setAuthentication(authentication);

			// Tạo session mới để lưu authentication
			HttpSession session = request.getSession(true);
			session.setAttribute("SPRING_SECURITY_CONTEXT", SecurityContextHolder.getContext());

			logger.info("User logged in: {}", user.getUsername());

			return ResponseEntity.ok(Map.of("message", "Đăng nhập thành công!", "username", user.getUsername(), "email",
					user.getEmail(), "redirectUrl", "/dashboard" // URL để redirect sau khi đăng nhập
			));
		} catch (Exception e) {
	...
	public ResponseEntity<?> sendForgotPasswordOtp(@Valid @RequestBody ForgotPasswordRequestDto dto,
	...
	public ResponseEntity<?> verifyForgotPasswordOtp(@Valid @RequestBody ForgotPasswordOtpDto dto,
			BindingResult result) {
		...
		return ResponseEntity.ok(Map.of("message", "Xác thực thành công! Đặt mật khẩu mới."));
	}
