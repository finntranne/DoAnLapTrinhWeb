...
public class CustomUserDetailsService implements UserDetailsService {
	...
	public UserDetails loadUserByUsername(String usernameOrEmail) throws UsernameNotFoundException {
		...
		UserDetails userDetails = org.springframework.security.core.userdetails.User.withUsername(user.getUsername())
				...
				.credentialsExpired(false).disabled(user.getStatus() != null && user.getStatus() != 1).build();
	...
	private Collection<? extends GrantedAuthority> mapRolesToAuthorities(Set<Role> roles) {
		if (roles == null || roles.isEmpty()) {
			...
			return Set.of(new SimpleGrantedAuthority("CUSTOMER")); // Mặc định là CUSTOMER nếu không có role
		}

		Set<GrantedAuthority> authorities = roles.stream().map(role -> {
			String roleName = role.getRoleName().toUpperCase().trim(); // Chỉ cần .toUpperCase().trim()
			logger.info("Mapping: {} -> {}", role.getRoleName(), roleName);
			return new SimpleGrantedAuthority(roleName); // <-- Trả về tên role trực tiếp
